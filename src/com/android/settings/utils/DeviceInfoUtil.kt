...
object DeviceInfoUtil {
    ...
    fun getProcessor(): String {
        ...
        return when (number) {
            ...
            8 -> "Google Tensor G3"
            9 -> "Google Tensor G4"
            else -> SystemProperties.get("ro.rvos.processor", "Unknown")
                .replace("_", " ")
        }
    }
        ...
        fun getTotalRam(): String {
            ...
            val totalRamBytes = memInfoReader.totalSize
            val totalRamGB = totalRamBytes / (1024.0 * 1024.0 * 1024.0)
            val roundedRamGB = roundToNearestKnownRamSize(totalRamGB).toDouble()

            val swapGB = getZramSize()

            return if (swapGB > 0) {
                String.format(Locale.US, "%.1f GB + %.2f GB ZRAM", roundedRamGB, swapGB)
            } else {
                String.format(Locale.US, "%.1f GB", roundedRamGB)
            }
        }

        fun getZramSize(): Double {
            val swapLine: String = try {
                BufferedReader(FileReader("/proc/meminfo")).use { reader ->
                    reader.lineSequence().firstOrNull { it.startsWith("SwapTotal:") } ?: return 0.0
                }
            } catch (e: IOException) {
                return 0.0
            }

            val swapValue = swapLine.split(":").getOrNull(1)?.trim()?.split(" ")?.getOrNull(0)

            return try {
                val swapValueKB = swapValue?.toIntOrNull() ?: return 0.0
                if (swapValueKB > 0) {
                    swapValueKB / (1024.0 * 1024.0)
                } else {
                    0.0
                }
